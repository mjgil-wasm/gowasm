import { testBrowserShellArchiveImport } from "./test-browser-shell-archive.js";
...
async function runAll() {
  try {
    await testBrowserShellArchiveImport({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser shell archive tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} browser shell archive failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
