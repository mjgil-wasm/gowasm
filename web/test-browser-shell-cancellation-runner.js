...
async function runAll() {
  try {
    await testBrowserShellCancellationRecovery({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser shell cancellation tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent =
      `${failed} browser shell cancellation failure(s), ${passed} assertions passed\n`
