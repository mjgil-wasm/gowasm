import { testBrowserShellCaches } from "./test-browser-shell-cache.js";
...
async function run() {
  try {
    await testBrowserShellCaches({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser shell cache tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} browser shell cache failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
