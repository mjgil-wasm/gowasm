import { testBrowserShellBootUrls } from "./test-browser-shell-boot.js";
...
async function runAll() {
  try {
    await testBrowserShellBootUrls({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser shell boot URL tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} browser shell boot URL failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
