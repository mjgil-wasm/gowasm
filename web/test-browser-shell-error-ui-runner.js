import { testBrowserShellErrorUi } from "./test-browser-shell-error-ui.js";
...
async function runAll() {
  try {
    await testBrowserShellErrorUi({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser shell error UI tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent =
      `${failed} browser shell error UI failure(s), ${passed} assertions passed\n`
