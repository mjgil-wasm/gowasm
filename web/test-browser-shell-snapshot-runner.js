import { testBrowserShellSnapshots } from "./test-browser-shell-snapshot.js";
...
async function runAll() {
  try {
    await testBrowserShellSnapshots({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser shell snapshot tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} browser shell snapshot failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
