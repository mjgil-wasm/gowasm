import { testPackagedExamples } from "./test-browser-shell-examples.js";
...
async function runAll() {
  try {
    await testPackagedExamples({ assert, frame, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all packaged browser example tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} packaged browser example failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
