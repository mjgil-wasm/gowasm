import { runIdeZipTests } from "./test-ide-zip.js";
...
async function runAll() {
  try {
    await runIdeZipTests({ assert, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all IDE ZIP tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} IDE ZIP failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
