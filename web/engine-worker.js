...
(async () => {
  const v = `?cb=${Date.now()}`;
  const [
    { buildCapabilityResumeRequest, CancelledRunError, disposeFetchSessions, formatError, withHostClock },
    { validateWasmBufferWindow, validateWorkerRequestEnvelope },
    { buildModuleResumeRequest },
  ] = await Promise.all([
