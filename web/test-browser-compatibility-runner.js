import { runBrowserCompatibilityTests } from "./test-browser-compatibility.js";
...
async function runAll() {
  try {
    await runBrowserCompatibilityTests({ assert, log });
  } catch (error) {
    failed += 1;
  ...
  if (failed === 0) {
    ...
    summary.textContent = `all browser compatibility tests passed (${passed} assertions)`;
  } else {
    summary.className = "fail";
    summary.textContent = `${failed} browser compatibility failure(s), ${passed} assertions passed\n${results.textContent.trim()}`;
  }
