use super::{StdlibFunction, SHA512_SUM512};
use crate::{Program, Value, ValueData, Vm, VmError};

pub(super) const SHA512_FUNCTIONS: &[StdlibFunction] = &[StdlibFunction {
    id: SHA512_SUM512,
    symbol: "Sum512",
    returns_value: true,
    handler: sha512_sum512,
}];

fn sha512_sum512(vm: &mut Vm, program: &Program, args: &[Value]) -> Result<Value, VmError> {
    if args.len() != 1 {
...
fn compute_sha512(data: &[u8]) -> [u8; 64] {
    const K: [u64; 80] = [
        0x428a2f98d728ae22,
        0x7137449123ef65cd,
        0xb5c0fbcfec4d3b2f,
...
pub(super) const SHA512_BLOCK_SIZE: i64 = 128;
pub(super) const SHA512_SIZE: i64 = 64;
