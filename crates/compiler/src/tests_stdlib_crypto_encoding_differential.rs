...
#[test]
fn crypto_encoding_differential_corpus_matches_checked_in_native_go_outputs() {
    let index = load_corpus_index();
    assert_eq!(
        index.schema_version, 1,
        "unexpected crypto/encoding differential schema"
    );
    assert!(
        !index.cases.is_empty(),
        "crypto/encoding differential corpus should contain representative cases"
    );
...
fn load_corpus_index() -> DifferentialCorpusIndex {
    serde_json::from_str(
        &fs::read_to_string(corpus_root().join("index.json"))
            .expect("crypto/encoding differential corpus index should be readable"),
    )
    .expect("crypto/encoding differential corpus index should deserialize")
}

fn corpus_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../testdata/crypto-encoding-differential")
}
...
fn run_case(case: DifferentialCase) {
    ...
    let program = compile_workspace(&source_inputs, &case.entry_path).unwrap_or_else(|_| {
        panic!(
            "crypto/encoding differential case `{}` should compile",
            case.name
        )
    });

    let mut vm = Vm::new();
    vm.run_program(&program).unwrap_or_else(|_| {
        panic!(
            "crypto/encoding differential case `{}` should run",
            case.name
        )
    });
    assert_eq!(
        ...
        "crypto/encoding differential case `{}` diverged from the checked-in native Go output",
        case.name
    );
}
    ...
    for path in workspace_files {
        ...
        files.push(WorkspaceFileFixture {
            ...
            contents: fs::read_to_string(&full_path).unwrap_or_else(|_| {
                panic!(
                    "crypto/encoding differential workspace file `{}` should be readable",
                    full_path.display()
                )
            }),
        });
